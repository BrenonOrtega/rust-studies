...
fn main() {
    ...
    //
    match eat(arg) {
        true => println!("Might be bananas"),
        false => println!("Not bananas")
    };
...
fn inspect(s: &str) {
    let message = if is_plural(s) { "plural" } else { "singular" };

    println!("The string {} is {}", s, message);
}

fn change(s: &mut String) {
    let is_plural = is_plural(&s);
