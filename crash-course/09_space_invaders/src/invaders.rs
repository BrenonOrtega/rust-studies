...
use crate::{NUMBER_COLUMNS, frame::{Drawable, Frame}, render::Updatable};
...
impl Invaders {
    pub fn new() -> Invaders {
        Invaders { list: Invader::create_level(2),
...
impl Drawable for Invaders {
    fn draw(&self, frame: &mut Frame) {
        ...
        let invader_render = if time_left / duration > 0.5 { "+" } else { "x" };
        
        self.list.iter()
            .for_each(|invader: &Invader| 
                frame[invader.x_index][invader.y_index] = invader_render);
    }
}
